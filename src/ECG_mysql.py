...
# Veriyi saklama
def save_encrypted_data(encrypted_data, timestamp):
    connection = create_db_connection()
    cursor = connection.cursor()
    for vec in encrypted_data:
        encrypted_bytes = vec.serialize()
        cursor.execute("INSERT INTO encrypted_ecg_data (encrypted_value, timestamp) VALUES (%s, %s)", (encrypted_bytes, timestamp))
    connection.commit()
    connection.close()
    print("Encrypted data saved to database")

# Veriyi okuma ve çözme
def load_encrypted_data(id):
    connection = create_db_connection()
    cursor = connection.cursor()
    cursor.execute("SELECT encrypted_value FROM encrypted_ecg_data WHERE id = %s", (id,))
...
def start_fhe_encrypt():
    ...
    save_encrypted_data(encrypted_data, timestamp)  # Şifrelenmiş veriyi sakla

def start_fhe_decrypt():
    global context
    id = simpledialog.askinteger("Input", "Enter the ID:")
    if id is not None:
        encrypted_data = load_encrypted_data(id)  # Veriyi yükle
        if encrypted_data:
            ...
            plot_data(stored_data, "Original Data")
...
encrypt_button = ttk.Button(button_frame, text="FHE Save", command=start_fhe_encrypt)
