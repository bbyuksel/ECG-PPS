...
timestamps = []
window_size = 3000
current_id = None  # Currently used ID
    ...
    while is_reading:
        if ser.in_waiting > 0:
            ...
            if data == b'\xF8':  # Wave sampling point marker
                ...
                stored_data.extend(list(wave_data))  # Save data
...
def update_plot(frame):
    if is_plotting:
        if len(ecg_data) > 0:
            ...
            num_ticks = 10
            tick_positions = np.linspace(0, len(original_data) - 1, num_ticks).astype(int)
            ax1.set_xticks(tick_positions)
            ax1.set_xticklabels([timestamps[pos] for pos in tick_positions], rotation=45, ha='right')

            ax2.set_xticks(tick_positions)
            ax2.set_xticklabels([timestamps[pos] for pos in tick_positions], rotation=45, ha='right')
    return line1, line2
...
root = tk.Tk()
root.title("Monitoring Real-Time ECG")
...
line2, = ax2.plot([], [], label='AES Decrypted Data')
ax1.set_ylim(-10, 300)
ax2.set_ylim(-10, 300)
ax1.set_title('Original Data')
ax2.set_title('AES Decrypted Data')
